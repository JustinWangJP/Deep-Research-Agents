...
// 翻訳値が空でないかチェックする関数
function checkTranslationValues(obj: Record<string, unknown>, prefix = '', errors: string[] = []): string[] {
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      const fullKey = prefix ? `${prefix}.${key}` : key;
      
      if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
        checkTranslationValues(obj[key] as Record<string, unknown>, fullKey, errors);
...
describe('Translation Completeness', () => {
  const namespaces = ['common', 'dashboard', 'agents', 'search', 'memory', 'citations', 'forms', 'errors'];
  
  namespaces.forEach(namespace => {
    test(`${namespace} namespace has same keys in both languages`, () => {
      const enKeys = extractKeys(translations.en[namespace as keyof typeof translations.en]);
      const jaKeys = extractKeys(translations.ja[namespace as keyof typeof translations.ja]);
    ...
    test(`${namespace} namespace has no empty translation values in English`, () => {
      const errors = checkTranslationValues(translations.en[namespace as keyof typeof translations.en]);
    ...
    test(`${namespace} namespace has no empty translation values in Japanese`, () => {
      const errors = checkTranslationValues(translations.ja[namespace as keyof typeof translations.ja]);
...
describe('Translation Structure Validation', () => {
  ...
  test('Common status keys are consistent', () => {
    const expectedStatusKeys = ['idle', 'running', 'completed', 'error', 'paused'];
  ...
  test('Navigation keys are consistent', () => {
    const expectedNavKeys = ['dashboard', 'agents', 'search', 'memory', 'citations'];
...
describe('Translation Content Validation', () => {
  test('No translation contains placeholder text', () => {
    ...
    const checkForPlaceholders = (obj: Record<string, unknown>, prefix = '', errors: string[] = []) => {
      for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
          const fullKey = prefix ? `${prefix}.${key}` : key;
          
          if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
            checkForPlaceholders(obj[key] as Record<string, unknown>, fullKey, errors);
          } else if (typeof obj[key] === 'string') {
    ...
    Object.keys(translations).forEach(lang => {
      Object.keys(translations[lang as keyof typeof translations]).forEach(namespace => {
        const errors = checkForPlaceholders(
          translations[lang as keyof typeof translations][namespace as keyof typeof translations.en],
          `${lang}.${namespace}`
        );
        
        if (errors.length > 0) {
          console.error(`Placeholders found in ${lang}.${namespace}:`, errors);
        }
  ...
  test('Interpolation variables are consistent between languages', () => {
    ...
    const checkInterpolationConsistency = (enObj: Record<string, unknown>, jaObj: Record<string, unknown>, prefix = '', errors: string[] = []) => {
      for (const key in enObj) {
        if (Object.prototype.hasOwnProperty.call(enObj, key) && Object.prototype.hasOwnProperty.call(jaObj, key)) {
          const fullKey = prefix ? `${prefix}.${key}` : key;
          
          if (typeof enObj[key] === 'object' && typeof jaObj[key] === 'object' && 
              enObj[key] !== null && jaObj[key] !== null && 
              !Array.isArray(enObj[key]) && !Array.isArray(jaObj[key])) {
            checkInterpolationConsistency(enObj[key] as Record<string, unknown>, jaObj[key] as Record<string, unknown>, fullKey, errors);
          } else if (typeof enObj[key] === 'string' && typeof jaObj[key] === 'string') {
            ...
            if (JSON.stringify(enVars) !== JSON.stringify(jaVars)) {
              errors.push(`Interpolation variables mismatch in ${fullKey}: EN(${enVars.join(',')}) vs JA(${jaVars.join(',')})`);
            }
          }
        }
      }
