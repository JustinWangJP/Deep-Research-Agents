...
class SearchQuery(BaseModel):
    ...
    temperature: Optional[TemperatureLevel] = Field(
        TemperatureLevel.BALANCED, description="Search temperature"
    )
...
class DocumentType(BaseModel):
    ...
    key_fields: List[str] = Field(
        default_factory=list, description="Key fields for this type"
    )
    content_fields: List[str] = Field(
        default_factory=list, description="Content fields for this type"
    )
...
class MemoryEntryCreate(BaseModel):
    ...
    source: str = Field(
        "system", min_length=1, max_length=100, description="Source of the entry"
    )
    memory_type: MemoryType = Field(
        MemoryType.SESSION, description="Memory storage type"
    )
...
class MemoryQuery(BaseModel):
    ...
    query: str = Field(..., min_length=1, max_length=1000, description="Search query")
    entry_types: Optional[List[EntryType]] = Field(
        None, description="Filter by entry types"
    )
...
class MemoryStats(BaseModel):
    ...
    entry_types: Dict[str, int] = Field(
        default_factory=dict, description="Entries by type"
    )
    sources: Dict[str, int] = Field(
        default_factory=dict, description="Entries by source"
    )
    memory_types: Dict[str, int] = Field(
        default_factory=dict, description="Entries by memory type"
    )
...
class CitationCreate(BaseModel):
    ...
    content: str = Field(
        ..., min_length=1, max_length=5000, description="Citation content"
    )
    source_title: str = Field(
        ..., min_length=1, max_length=500, description="Source title"
    )
    source_url: Optional[str] = Field(None, description="Source URL")
    case_number: Optional[str] = Field(
        None, max_length=100, description="Case identifier"
    )
...
class CitationUpdate(BaseModel):
    ...
    content: Optional[str] = Field(
        None, min_length=1, max_length=5000, description="Citation content"
    )
    source_title: Optional[str] = Field(
        None, min_length=1, max_length=500, description="Source title"
    )
    source_url: Optional[str] = Field(None, description="Source URL")
    case_number: Optional[str] = Field(
        None, max_length=100, description="Case identifier"
...
class ConfigInfo(BaseModel):
    ...
    search_providers: List[SearchProviderInfo] = Field(
    ...
    supported_languages: List[str] = Field(
        default_factory=lambda: ["en", "ja"], description="Supported languages"
    )
...
class ResearchTaskCreate(BaseModel):
    ...
    temperature: TemperatureLevel = Field(
        TemperatureLevel.BALANCED, description="Research temperature"
    )
    max_iterations: int = Field(
        3, ge=1, le=10, description="Maximum quality iterations"
    )
...
class ResearchTask(BaseModel):
    ...
    progress: float = Field(
        0.0, ge=0.0, le=100.0, description="Task progress percentage"
    )
