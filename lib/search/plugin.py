...
class ModularSearchPlugin:
        ...
        if getattr(self, "_internal_functions_enabled", False):
            ...
            if not hasattr(self, "_search_internal_all_documents_decorated"):
                ...
                async def wrapped_search_internal_all_documents(
                    ...
                    return await original_method(query, top_k_per_source, use_hybrid_search, use_semantic_search)
            ...
            if hasattr(self, "_search_internal_all_documents_decorated"):
                ...
                if hasattr(self, "_original_search_internal_all_documents"):
                    setattr(
                        self,
                        "search_internal_all_documents",
                        self._original_search_internal_all_documents,
                    )
    ...
    def _generate_dynamic_functions(self):
        ...
        try:
            ...
            if hasattr(self.config, "project_config") and self.config.project_config:
                ...
                logger.error("Project configuration is required for ModularSearchPlugin")
    ...
    def _create_search_function(self, doc_type_config):
        ...
        if hasattr(self.config, "project_config") and self.config.project_config:
            search_example_config = self.config.project_config.get_search_example(doc_type_config.name)
        ...
        if hasattr(doc_type_config, "func_description") and doc_type_config.func_description:
            ...
            logger.debug(
                f"Function {func_name}: Using func_description from config: '{doc_type_config.func_description}'"
            )
        else:
        ...
        logger.debug(
            f"Function {func_name}: Added technical description with {len(doc_type_config.key_fields)} filterable fields"
        )
        ...
        logger.debug(
            f"Function {func_name}: Description parts - Main: {len(description_parts['main'])} chars, "
        ...
        async def search_function(
            ...
            if top_k > max_limit:
                logger.warning(f"top_k ({top_k}) exceeds max_results_limit ({max_limit}), using {max_limit}")
                top_k = max_limit
        ...
        decorated_function = kernel_function(name=func_name, description=description)(search_function)
        ...
        if search_example_config and search_example_config.get("query_examples"):
            logger.debug(
                f"Function {func_name} configured with query examples: {search_example_config['query_examples']}"
            )
    ...
    async def search_internal_all_documents(
        ...
        try:
            ...
            search_example_config = None
            if hasattr(self.config, "project_config") and self.config.project_config:
                search_example_config = self.config.project_config.get_search_example("all_documents")
            ...
            if top_k_per_source > max_limit:
                logger.warning(
                    f"""top_k_per_source ({top_k_per_source}) exceeds max_results_limit ({
            ...
            results = await self.search_manager.search_internal_all(search_query, top_k_per_source)
    ...
    async def web_search_impl(
        ...
        try:
            ...
            logger.info(
                f"Web search completed - Found {len(processed_results)} results (including images if requested)"
            )
    ...
    async def web_search(
        ...
        """
        return await self.web_search_impl(query, top_k, time_range, topic, search_depth, include_image_descriptions)
