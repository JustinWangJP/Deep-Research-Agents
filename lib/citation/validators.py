...
class CitationValidator:
    ...
    def is_valid_internal_source(result: dict[str, Any]) -> bool:
        ...
        if content_path and any(
            indicator in content_path.lower() for indicator in ["http://", "https://", "www.", ".com", ".org"]
        ):
        ...
        if document_title and any(
            indicator in document_title.lower() for indicator in ["http", "www", ".com", ".org", "external", "web"]
        ):
            logger.warning(f"Rejected external document title: {document_title}")
            return False
    ...
    @staticmethod
    def validate_citations_batch(citations: list[dict[str, Any]]) -> dict[str, Any]:
        """
        ...
        for i, citation in enumerate(citations, 1):
            ...
            if errors:
                invalid_count += 1
                all_errors.extend([f"Citation {i}: {error}" for error in errors])
            else:
                valid_count += 1
