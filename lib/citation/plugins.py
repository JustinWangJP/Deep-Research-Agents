...
class CitationPlugin:
    ...
    async def add_citation(
        ...
        """Add a new citation."""
        page_num = int(page_number) if page_number and page_number.isdigit() else None
    ...
    @kernel_function(
        name="add_multiple_citations",
        description="Add multiple citations at once from structured citation data",
    )
    async def add_multiple_citations(
        self,
        citations_data: Annotated[
            list[dict[str, Any]],
            "List of citation dictionaries with keys: content, source_title, case_number (optional), page_number (optional), confidence (optional)",
        ],
    ) -> Annotated[str, "Success message with count of created citations"]:
        """Add multiple citations from structured dictionary data."""
        try:
            created_ids = self.manager.create_multiple_citations(citations_data)
    ...
    @kernel_function(
        name="create_citations_from_search_batch",
    ...
    async def create_citations_from_search_batch(
        ...
        search_results: Annotated[
            list[dict[str, Any]],
            "List of internal AI search result dictionaries containing fields like content_text, document_title, search_type, record_id, etc.",
        ],
        ...
        try:
            created_ids = self.manager.create_citations_from_search_batch(search_results, batch_name)
    ...
    async def import_search_results(
        ...
        search_results: Annotated[
            list[dict[str, Any]],
            "List of internal AI search result dictionaries to be imported as citations",
        ],
    ...
    @kernel_function(
        name="generate_citation_list",
        description="Generate formatted citation list for reports from internal sources",
    )
    async def generate_citation_list(
        self,
    ) -> Annotated[str, "Formatted citation list containing only internal source references"]:
        """Generate formatted citation list for reports."""
        return self.manager.generate_citation_list()

    @kernel_function(name="clear_citations", description="Clear all citations")
