...
class ResearchExecutor:
    ...
    def _create_research_agent(self, config_dict: dict, memory_plugin: Any = None) -> ChatCompletionAgent:
        """
        ...
        # Create service
        service = get_azure_openai_service(self.config.get_model_config("gpt41"))

        # Get instructions based on temperature setting
        if temp is not None:
            instructions = TemperatureManager.get_temperature_instructions(temp, approach)
        ...
        if memory_plugin:
            ...
            self.logger.debug(
                f"⚠️ [MEMORY] No memory plugin available for {agent_suffix} - important findings may not be preserved"
            )
    ...
    def _prepare_agents(
        ...
        """
        temperature_configs = TemperatureManager.get_temperature_configs(use_temperature_variation)

        if memory_plugin:
            self.logger.debug(f"💾 [MEMORY] Preparing {target_count} agents with shared memory capabilities")
        else:
            self.logger.debug(
                f"⚠️ [MEMORY] Preparing {target_count} agents without memory capabilities - important findings may not be preserved"
            )
        ...
        if use_temperature_variation:
            ...
            if len(research_agents) != target_count:
                ...
                for i in range(target_count):
                    agent = self._create_research_agent(temperature_configs[i], memory_plugin)
            ...
            if len(research_agents) >= target_count:
                ...
                self.logger.debug(
                    f"🔬 [ThreadPool] Creating {target_count - len(research_agents)} additional agents with memory"
                )
    ...
    async def _run_single_agent_with_thread(self, agent: Any, user_input: str, temperature: float | None = None) -> str:
        """
        ...
        try:
            ...
            self.logger.info(
                f"✅ [ThreadBased] {agent.name} completed with {message_count} messages, final content: {len(final_content)} chars"
            )
            if final_content:
    ...
    def _build_kernel_arguments(self, agent: Any, temperature: float | None) -> Any | None:
        """
    ...
    async def _get_agent_response(self, agent: Any, user_input: str, kernel_arguments: Any | None) -> Any:
        """
    ...
    async def _process_agent_response(self, agent: Any, response: Any) -> tuple[str, int]:
        ...
        if thread:
            try:
                    ...
                    if hasattr(message, "items") and message.items:
                        for item in message.items:
                            if hasattr(item, "function_name") and hasattr(item, "arguments"):
                                ...
                                self.logger.debug(
                                    f"🔧 [TOOL RESULT] {agent.name} received result from: {item.function_name}"
                                )
                                self.logger.debug(f"🔧 [TOOL RESULT] Result preview: {str(item.result)[:200]}...")
                                complete_content_parts.append(
                ...
                self.logger.error(
                    f"🧵 [THREAD] Error getting messages: {get_messages_error}",
                    exc_info=True,
                )
    ...
    def _synthesize_results(
        ...
        if use_temperature_variation:
            synthesized_parts.append("# 🌡️ Temperature Variation Parallel Research Results - Lead Researcher Analysis")
            ...
            synthesized_parts.append("Integrated results from parallel investigation by 3 research agents.\n")
        ...
        for i, agent_result in enumerate(results, 1):
            if isinstance(agent_result, Exception):
                ...
                if use_temperature_variation:
                    ...
                    synthesized_parts.append(f"## Research Perspective {i} - RESEARCHER{i}")
                    synthesized_parts.append(f"Error: {str(agent_result)}")
                continue
            ...
            if use_temperature_variation:
                self.logger.debug(
                    f"🌡️ Processing temperature {temp_config['temp']} result: {len(str(agent_result)) if agent_result else 0} chars"
                )
                synthesized_parts.append(
                    f"""### 🌡️ Temperature {
                                         temp_config['temp']} - {temp_config['approach']}"""
                )
                synthesized_parts.append(f"**Approach**: {temp_config['approach']}")
                ...
                self.logger.debug(
                    f"📊 Processing standard agent {i} result: {len(str(agent_result)) if agent_result else 0} chars"
                )
                synthesized_parts.append(f"## Research Perspective {i} - RESEARCHER{i}")
                synthesized_parts.append(str(agent_result) if agent_result else "No content generated")
        ...
        if use_temperature_variation:
            ...
            if self.project_config:
                quality_indicators = self.project_config.get_quality_indicators("temperature_variation")
                for indicator in quality_indicators:
                    formatted_indicator = indicator.format(agent_count=len(temperature_configs))
    ...
    def _get_error_message(self, user_input: str, error: str, use_temperature_variation: bool) -> str:
        """Get formatted error message using project config templates if available."""
        if self.project_config:
            if use_temperature_variation:
                template = self.project_config.get_error_template("temperature_variation")
                if template:
                    return template.format(user_input=user_input, error_message=error)
            else:
                template = self.project_config.get_error_template("standard_parallel")
                if template:
                    return template.format(user_input=user_input, error_message=error)
...
An error occurred during parallel research agent execution: {error}"""
    ...
    async def execute_concurrent_research(
        ...
        """
        self.logger.info(
            f"""🔬 [ThreadPool] Starting concurrent research for query: {
                         user_input[:100]}..."""
        )
        ...
        if memory_plugin:
            self.logger.info(
                f"💾 [MEMORY] Found memory plugin - research agents will have knowledge preservation capabilities"
            )
        else:
            self.logger.debug(
                f"⚠️ [MEMORY] No memory plugin found - research agents will not be able to preserve important findings"
            )

        # Prepare agents for execution
        agents_to_use = self._prepare_agents(research_agents, use_temperature_variation, memory_plugin)
        ...
        if agents_with_memory < len(agents_to_use):
            self.logger.debug(
                f"⚠️ [MEMORY] {len(agents_to_use) - agents_with_memory} agents lack memory capabilities - important findings may not be preserved"
            )
        ...
        try:
            ...
            if use_temperature_variation:
                tasks = [
                    self._run_single_agent_with_thread(agent, user_input, temperature_configs[i]["temp"])
            ...
            synthesized_result = self._synthesize_results(
                results, temperature_configs, user_input, use_temperature_variation
            )
            ...
            self.logger.error(
                f"❌ [ThreadPool] Error in concurrent research execution: {str(e)}",
                exc_info=True,
            )
            return self._get_error_message(user_input, str(e), use_temperature_variation)
    ...
    def _validate_temperature_settings(self, agents_to_use: list[Any], temperature_configs: list[dict]):
        ...
        for i, (agent, config) in enumerate(zip(agents_to_use, temperature_configs)):
            ...
            if temp is not None:
                self.logger.debug(
                    f"🌡️ [TEMP VALIDATION] Agent {i+1} ({agent.name}): temperature={temp}, approach={approach}"
                )
                ...
                self.logger.debug(
                    f"🔬 [TEMP VALIDATION] Agent {i+1} ({agent.name}): standard temperature (no specific setting)"
                )
    ...
    def _get_agent_plugin_info(self, agent: Any) -> dict:
        ...
        if hasattr(agent, "kernel") and agent.kernel.plugins:
            ...
            return {"plugin_names": [], "has_memory": False, "plugin_count": 0}
        ...
        for agent in agents:
            ...
            if plugin_info["has_memory"]:
                ...
                memory_plugin_name = next(
                    (
                        name
                        for name in plugin_info["plugin_names"]
