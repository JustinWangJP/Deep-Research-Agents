...
class PromptManager:
        ...
        try:
            ...
            if hasattr(company, "name"):
                ...
                company_display_name = (
                    company.get("display_name", "Organization") if isinstance(company, dict) else "Organization"
                )
    ...
    def get_domain_concepts_section(self) -> str:
        ...
        return "**CORE CONCEPT FOCUS**: Use specific terms: " + "; ".join(concepts) if concepts else ""
    ...
    def get_credibility_scores_section(self) -> str:
        ...
        for (
            criterion,
            description,
        ) in self.config.credibility.evaluation_criteria.items():
            criteria.append(f"- **{criterion.replace('_', ' ').title()}**: {description}")
...
- **Metadata Preservation**: Include document titles, case numbers, {self.get_record_id_field()}
    ...
    def get_search_examples_by_type(self, document_type: str) -> dict[str, Any]:
    ...
    def get_search_examples_section(self) -> str:
        ...
        for name, example in self.config.search_examples.items():
            ...
            example_queries = ", ".join([f'"{q}"' for q in example.query_examples[:2]])

            line = f"""- Function: `{
                example.function_name}` - Query examples: {example_queries}"""

            # Use generic categorization instead of business-specific keywords
            if any(keyword in name for keyword in ["category", "type_a", "type_b"]):
        ...
        if category_examples:
            sections.append("**Category-Related Queries with Semantic Understanding**:")
            sections.extend(category_examples)
            sections.append(f"- Parameters: {params_str}")
            sections.append("")

        if research_examples:
            sections.append("**Research and Knowledge Queries with Conceptual Matching**:")
            sections.extend(research_examples)
            sections.append(f"- Parameters: {params_str}")
            sections.append("")

        if comprehensive_examples:
            sections.append("**Comprehensive Coverage with Hybrid Capabilities**:")
