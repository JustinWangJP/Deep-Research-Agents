"""
Logging configuration utilities for the T&E Research Agent.
...
def setup_agent_activity_logging(agent_name: str, log_dir: str | None = None) -> logging.Logger:
    """
...
def log_system_info(logger: logging.Logger):
    ...
    logger.info("=" * 50)
    logger.info("T&E Research Agent - System Information")
    logger.info("=" * 50)
...
def create_performance_logger() -> logging.Logger:
    ...
    handler = logging.handlers.RotatingFileHandler(perf_file, maxBytes=5 * 1024 * 1024, backupCount=3, encoding="utf-8")

    formatter = logging.Formatter("%(asctime)s - PERF - %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
    handler.setFormatter(formatter)
